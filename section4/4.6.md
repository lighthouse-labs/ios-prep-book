...
* The first thing you're going to do is create a new object with the type `NSString`. Call this object `name` and set its value to be your name using a string literal.
...
`NSString` is immutable. So any method that appears to alter a `NSString` actually creates a new one with the change and returns it. Some methods that do this are:
...
* Test out each of the methods above on your name variable and print the results.
